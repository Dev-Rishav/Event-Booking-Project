...
export const createBooking = async (req, res) => {
  ...
  try {
    ...
    // Lock and verify seat availability in database
    for (const seat of seatNumbers) {
      const lockQuery = `
        SELECT * FROM seats 
...
cron.schedule("*/5 * * * *", async () => {
  ...
  try {
    ...
    for (const payment of result.rows) {
      const { transaction_id } = payment;

      // Step 1: Find associated seat bookings (if any were tentatively held)
      const bookings = await pool.query(
        `SELECT seat_id, show_id FROM bookings WHERE transaction_id = $1`,
        [transaction_id]
      );

      // Step 2: Mark payment as expired
      await pool.query(
        `UPDATE payment SET status = 'expired' WHERE transaction_id = $1`,
        [transaction_id]
      );

      // Step 3: Release seats in DB
      if (bookings.rowCount > 0) {
        const seatIds = bookings.rows.map(row => row.seat_id);
        ...
        // Step 4: Delete Redis holds
        for (const row of bookings.rows) {
          const redisKey = `hold:${row.show_id}:${row.seat_id}`;
          await redisClient.del(redisKey);
        }

        // Step 5: Delete tentative bookings
        await pool.query(
          `DELETE FROM bookings WHERE transaction_id = $1`,
          [transaction_id]
        );
      }

      console.log(`Expired and cleaned payment: ${transaction_id}`);
    }
  } catch (err) {
...
// /**
//  * STEP 1: Hold seats temporarily in Redis (UX purpose)
//  */
// export const holdSeats = async (req, res) => {
//   const { show_id, seats, userId } = req.body;
...
// /**
//  * STEP 2: Create PayPal Payment & Lock seats in DB
//  * - Lock rows with FOR UPDATE
//  * - Insert only payment record
//  */
...
//     // Verify user still holds seats in Redis
//     for (const seat of seatNumbers) {
//       const redisKey = `hold:${showId}:${seat}`;
//       const heldBy = await redisClient.get(redisKey);
...
//     // Lock seats in DB to prevent concurrent booking
//     for (const seat of seatNumbers) {
//       const lockQuery = `
//         SELECT * FROM seats 
//         WHERE show_id = $1 AND seat_number = $2 
...
//         // Insert only into payment table with pending status
//         await pool.query(
//           `INSERT INTO payment (transaction_id, amount, status, user_id)
//            VALUES ($1, $2, 'pending', $3)`,
//           [paymentId, totalAmount, userId]
//         );
...
//         res.status(500).json({ success: false, message: "DB error during payment creation" });
//       }
//     });
//   } catch (err) {
//     await pool.query('ROLLBACK');
//     console.error("Transaction error:", err);
//     res.status(500).json({ success: false, message: "Unexpected transaction error" });
//   }
// };


// /**
//  * STEP 3: Capture Payment & Commit Bookings
//  * - Only after payment success, insert bookings and update seats
//  */
// export const capturePayment = async (req, res) => {
//   const { paymentId, PayerID, userId, showId, selectedSeats } = req.body;
...
//           // Insert booking
//           const bookingResult = await pool.query(
//             `INSERT INTO bookings (user_id, show_id, seat_id, transaction_id, payment_status)
//              VALUES ($1, $2, $3, $4, 'paid')
//              RETURNING booking_id`,
//             [userId, showId, seat.seat_id, paymentId]
//           );
...
//           // Cleanup Redis hold
//           const redisKey = `hold:${showId}:${seat.seat_number}`;
//           await redisClient.del(redisKey);
//         }
...
//         res.status(200).json({
//           success: true,
//           message: "Payment captured, seats booked",
//           bookingIds,
//           transaction_id: paymentId
//         });
...
//         await pool.query('ROLLBACK');
//         console.error("DB error:", dbError);
//         res.status(500).json({ message: "Booking failed after payment capture" });
//       }
//     });
//   } catch (err) {
//     console.error("Unexpected error:", err);
