...
export const createOrganizerSubscription = async (req, res) => {
  ...
  try {
    ...
    const { email, price, plan_id, start_date, end_date } = req.body.email;
    ...
    paypal.payment.execute(paymentId, { payer_id: PayerID }, async (error, payment) => {
      ...
      try {
        ...
        const result = await pool.query(
          `SELECT subscription_id FROM organizer_subscriptions WHERE organizer_email = $1`,
          [email]
        );
        
        const subscription_id = result.rows[0]?.subscription_id;
...
export const setStatusInactiveOfPlan = async (req, res) => {
  const plan_id = req.params.id;
  try {
    const result = await Subscription.setStatusInactiveOfPlan(plan_id);
  } catch (error) {
...
export const setStatusActiveOfPlan = async (req, res) => {
  const plan_id = req.params.id;
  try {
    const result = await Subscription.setStatusActiveOfPlan(plan_id);
  } catch (error) {
    console.error("Error updating plans:", error.message);
    res.status(500).json({
      success: false,
...
export const getCurrentSubscriptionOfOrganizer = async (req, res) => {
  const organizer_email = req.params.id;
  try {
    const result = await Subscription.getCurrentSubscriptionOfOrganizer(organizer_email);
    return res.status(200).json({ msg: "plan : ", result });
  } catch (error) {
    console.error("Error fetching subscription plan:", error.message);
    res.status(500).json({
