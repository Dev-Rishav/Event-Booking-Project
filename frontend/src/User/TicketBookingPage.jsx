...
const TicketBookingPage = () => {
  ...
  useEffect(() => {
    if (selectedUserShow) {
      ...
      if (selectedUserShow.event_category === "Sports") setVenueType("stadium");
      else if (selectedUserShow.event_category === "Conference") setVenueType("conference");
  ...
  const handlePayNow = () => {
    ...
    createBookingAndRedirect({
      ...
      selectedSeats: selectedSeats.map(seatNo => {
        ...
        return { seat_number: seatNo, seat_id: seat?.seat_id, price: seat?.price };
      }),
  ...
  const getSeatColor = (category, status, isSelected) => {
    ...
    if (isSelected) return holdTimer ? "bg-indigo-600 text-white" : "bg-green-600 text-white";
    const colors = {
    ...
    return colors[(category || "").toLowerCase()] || "bg-gray-300 hover:bg-gray-400 text-gray-800";
  };
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {seats.map(seat => {
          ...
          return (
            <motion.button
              key={seat.seat_id}
              onClick={() => seat.status !== "booked" && handleSelectSeat(seat.seat_number)}
              className={`w-12 h-12 rounded-lg shadow-md border-2 border-gray-200 flex flex-col items-center justify-center cursor-pointer transition-all duration-200 font-medium ${getSeatColor(seat.seat_category, seat.status, isSelected)} ${seat.status === "booked" ? "cursor-not-allowed" : "hover:shadow-lg"} ${isSelected ? "ring-2 ring-offset-2 ring-blue-500" : ""}`}
  ...
  const renderSeatLayout = () => {
    if (loading) return <div className="text-center py-12">Loading seat map...</div>;
    if (!seats.length) return <div className="text-center py-12">No seats available for this event</div>;
    return venueType === "theatre" ? renderTheatreLayout() : null;
  };
  ...
  return (
    ...
    >
      <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-center">🎟️ Book Your Tickets</h1>
      ...
      <div className="flex justify-center gap-3 mb-8 flex-wrap">
        {["VIP", "Premium", "Standard", "Economy", "Selected", "Held", "Booked"].map(label => (
          <div key={label} className="flex items-center px-3 py-1 bg-white dark:bg-gray-800 rounded-full shadow-xs border">
            <div className={`w-3 h-3 rounded-full mr-2 ${label === "Selected" ? "bg-green-600" : label === "Held" ? "bg-indigo-600" : label === "Booked" ? "bg-red-400" : label === "VIP" ? "bg-purple-600" : label === "Premium" ? "bg-blue-600" : label === "Standard" ? "bg-yellow-500" : "bg-gray-600"}`}></div>
            <span className="text-xs sm:text-sm">{label}</span>
          </div>
        ))}
      </div>

      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md p-4 sm:p-6 mb-8">{renderSeatLayout()}</div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md p-4 sm:p-6 mb-8">
          ...
          <div className="flex overflow-x-auto pb-4 gap-4">
            {selectedSeats.map(seatNo => {
              const seat = seats.find(s => s.seat_number === seatNo);
              return (
                <div key={seatNo} className="flex-shrink-0 w-48 h-32 bg-gradient-to-br from-blue-50 to-gray-50 dark:from-gray-800 dark:to-gray-700 border rounded-lg p-4 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex justify-between items-center pt-4 border-t mt-4">
            ...
            <span className="text-xl font-bold text-blue-600">${calculateTotalAmount().toFixed(2)}</span>
          </div>
        </div>
      )}
